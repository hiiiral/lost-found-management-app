...
class _ManageItemsScreenState extends State<ManageItemsScreen> {
  ...
  int get _unclaimedItems =>
      _items.where((item) => item.status == 'New' || item.status == 'Stored').length;

  int get _returnedItems =>
      _items.where((item) => item.status == 'Claimed' || item.status == 'Matched').length;

  int get _highPriorityItems => _items.where((item) => item.status == 'New').length;
  ...
  Future<void> _showAddItemForm() async {
    ...
    if (created == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add item canceled')),
      );
      return;
    }
