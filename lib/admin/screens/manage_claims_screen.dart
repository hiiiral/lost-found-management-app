...
class _ManageClaimsScreenState extends State<ManageClaimsScreen> {
  ...
  int get _pendingCount =>
      _claims.where((claim) => claim.status == 'Pending').length;

  int get _approvedCount =>
      _claims.where((claim) => claim.status == 'Approved').length;

  int get _rejectedCount =>
      _claims.where((claim) => claim.status == 'Rejected').length;

  int get _reviewCount =>
      _claims.where((claim) => claim.status == 'Review').length;
  ...
  List<int> get _filteredClaimIndexes {
    ...
    return List<int>.generate(_claims.length, (index) => index).where((index) {
      ...
      final matchesStatus =
          _selectedStatus == 'All' || claim.status == _selectedStatus;
  ...
  Future<void> _showClaimReview(int index) async {
    ...
    _showMessage(
      'Opened review for ${claim.itemName}',
