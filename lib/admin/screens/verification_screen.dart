...
class _VerificationScreenState extends State<VerificationScreen> {
  final List<_VerificationEntry> _entries = [
    const _VerificationEntry(
      ...
      notes:
          'Serial number matches the stored device and handover can proceed.',
    ),
    const _VerificationEntry(
      ...
      notes:
          'Need a close-up wrist photo or purchase receipt for final confirmation.',
    ),
    const _VerificationEntry(
  ...
  int get _pendingChecks =>
      _entries.where((entry) => entry.status == 'Pending').length;

  int get _verifiedToday =>
      _entries.where((entry) => entry.status == 'Verified').length;

  int get _needReview => _entries
      .where((entry) => entry.status == 'Issue' || entry.status == 'Review')
      .length;
  ...
  List<int> get _filteredEntryIndexes {
    ...
    return List<int>.generate(_entries.length, (index) => index).where((index) {
      ...
      final matchesStatus =
          _selectedStatus == 'All' || entry.status == _selectedStatus;
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: Theme.of(
              context,
            ).textTheme.bodyMedium?.copyWith(color: const Color(0xFF6B7C93)),
          ),
          const SizedBox(height: 8),
          Text(
